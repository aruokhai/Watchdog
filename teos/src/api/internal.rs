...
mod tests_private_api {
    ...
    async fn test_get_appointments_watcher_with_userid() {
        ...
        for i in 0..3 {
            ...
            // Ensure that duplicate appointments are not stored for a user

            // Create a specific user
            let (random_user_sk, random_user_pk) = get_random_keypair();
            let random_user_id = UserId(random_user_pk);
            internal_api.watcher.register(random_user_id).unwrap();

            for _ in 0..appointments_to_create {
                let appointment = generate_dummy_appointment(Some(&dispute_txid)).inner;
                let signature = cryptography::sign(&appointment.to_vec(), &random_user_sk).unwrap();
                internal_api
                    .watcher
                    .add_appointment(appointment, signature)
                    .unwrap();
